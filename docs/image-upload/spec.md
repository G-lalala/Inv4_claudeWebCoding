# 画像アップロード機能 仕様書

## 概要
ユーザーが複数の画像を選択・アップロードし、フロントエンドで圧縮してからサーバーに送信する機能。アップロード後は確認画面で画像一覧を表示する。

## 要件

### 対応画像枚数
- **最小**: 3枚
- **最大**: 50枚
- バリデーション: 枚数が範囲外の場合はエラーメッセージを表示

### 対応画像フォーマット
- iOS形式: HEIC、HEIF
- Android形式: JPG、JPEG、PNG、WebP
- その他: BMP、GIF（静止画として扱う）

### 画像処理

#### フロントエンド圧縮
- **目的**: ユーザーの快適な操作体験を確保
- **処理タイミング**: 画像選択後、アップロード前
- **圧縮方法**:
  - HEIC/HEIF形式はJPEGに変換
  - 長辺を1920px以下にリサイズ（アスペクト比維持）
  - JPEG品質: 85%
  - 目標ファイルサイズ: 1枚あたり500KB〜1MB程度

#### バリデーション
- **ファイル形式チェック**: MIMEタイプと拡張子の両方で検証
- **ファイルサイズチェック**: 圧縮前10MB/枚、圧縮後2MB/枚
- **画像の正当性チェック**: 破損していないか確認

### 確認画面

#### 表示内容
- アップロードした画像のサムネイル一覧（グリッドレイアウト）
- 各画像のファイル名、サイズ表示
- 合計枚数の表示
- 削除ボタン（個別削除可能）
- 画像の並び替え機能（ドラッグ&ドロップ）

#### アクション
- **戻るボタン**: 画像選択画面に戻る
- **動画を生成ボタン**: 次の処理（スライドショー生成）へ進む

## 技術仕様

### アップロード方式の選択

#### 方式1: 直接アップロード（シンプル）
- フロントエンドから圧縮済み画像をサーバーに送信
- サーバーがローカルディスク（/tmp）に保存
- **メリット**: 実装がシンプル、インフラ要件が少ない
- **デメリット**: 大量ファイル時にサーバー負荷が高い、スケーラビリティに課題

#### 方式2: 署名付きURL + S3/LocalStack（推奨）
- フロントエンドが署名付きURLをサーバーから取得
- 圧縮済み画像をS3（開発環境ではLocalStack）に直接アップロード
- **メリット**: サーバー負荷軽減、スケーラブル、本番環境と同等の構成
- **デメリット**: インフラ設定が必要（LocalStackのセットアップ）

**推奨**: 方式2（署名付きURL + S3/LocalStack）を採用

### フロントエンド

#### 使用ライブラリ
- **画像圧縮**: `browser-image-compression` または `compressorjs`
- **HEIC変換**: `heic2any`
- **ドラッグ&ドロップ**: `react-beautiful-dnd` または HTML5 Drag and Drop API
- **AWS SDK**: `@aws-sdk/client-s3` または `@aws-sdk/s3-request-presigner`（方式2の場合）

#### 実装ファイル
- `app/components/ImageUploader.tsx`: 画像選択・アップロード UI
- `app/components/ImagePreview.tsx`: 確認画面 UI
- `app/utils/imageCompression.ts`: 画像圧縮ロジック
- `app/utils/imageValidation.ts`: バリデーションロジック
- `app/utils/s3Upload.ts`: S3署名付きURLアップロード処理（方式2）

### バックエンド

#### API エンドポイント

**方式1（直接アップロード）の場合**:
- `POST /api/upload`: 圧縮済み画像を受け取り、一時ディレクトリに保存
  - リクエスト: multipart/form-data
  - レスポンス: `{ sessionId: string, imageIds: string[] }`

**方式2（署名付きURL）の場合**（推奨）:
- `POST /api/upload/presigned-urls`: 署名付きURLを生成
  - リクエスト: `{ count: number, fileNames: string[] }`
  - レスポンス: `{ sessionId: string, uploadUrls: Array<{ id: string, url: string, key: string }> }`
- `POST /api/upload/complete`: アップロード完了を通知
  - リクエスト: `{ sessionId: string, imageKeys: string[] }`
  - レスポンス: `{ success: true }`

#### ストレージ

**方式1の場合**:
- **保存先**: `/tmp/uploads/{session-id}/`
- **ファイル名**: `{uuid}.jpg`
- **メタデータ**: `metadata.json`（画像順序、元のファイル名等）

**方式2の場合**（推奨）:
- **ストレージ**: S3バケット（開発: LocalStack、本番: AWS S3）
- **バケット名**: `slideshow-uploads`
- **キー形式**: `{session-id}/{uuid}.jpg`
- **メタデータ**: S3オブジェクトメタデータまたは別途DynamoDB/JSONファイル
- **有効期限**: 署名付きURLは5分、アップロード済みファイルは24時間後に削除

### LocalStack セットアップ（方式2の場合）

**詳細は `docs/LOCALSTACK_SETUP.md` を参照してください。**

#### 概要
- `docker-compose.yml`にLocalStackサービスを追加済み
- 初期化スクリプトで自動的にS3バケット作成、CORS設定、ライフサイクルルール設定
- `localstack/init/setup-s3.sh`がコンテナ起動時に自動実行

#### 起動コマンド
```bash
docker-compose up -d localstack
```

#### 動作確認
```bash
# ヘルスチェック
curl http://localhost:4566/_localstack/health

# バケット確認
docker-compose exec app sh -c "aws --endpoint-url=http://localstack:4566 s3 ls"
```

#### 重要な考慮事項
- **Docker Socketマウント**: 開発環境のみ（セキュリティリスクあり）
- **ポート4566**: ホストマシンで空いている必要あり
- **データ永続化**: `localstack/data/`に保存（.gitignore対象）
- **Windows環境**: Docker Socketパスやシェルスクリプトの改行コードに注意

詳細は `docs/LOCALSTACK_SETUP.md` を参照してください。

## エラーハンドリング

### クライアントサイド
- 枚数不足/超過: 「画像は3枚以上50枚以下でアップロードしてください」
- 非対応フォーマット: 「対応していないファイル形式です」
- ファイルサイズ超過: 「ファイルサイズが大きすぎます」
- 圧縮失敗: 「画像の処理に失敗しました。再度お試しください」

### サーバーサイド
- アップロード失敗: 500エラー、リトライ可能メッセージ
- ディスク容量不足: 503エラー、一時的なサービス停止通知

## アップロードフロー比較

### 方式1: 直接アップロード
```
1. ユーザーが画像を選択
2. フロントエンドで圧縮処理
3. multipart/form-dataでサーバーに送信
4. サーバーが/tmpに保存
5. レスポンス返却
```

### 方式2: 署名付きURL（推奨）
```
1. ユーザーが画像を選択
2. フロントエンドで圧縮処理
3. サーバーに署名付きURL発行をリクエスト（POST /api/upload/presigned-urls）
4. サーバーがS3署名付きURLを生成して返却
5. フロントエンドが各画像をS3に直接アップロード（PUT）
6. 全アップロード完了後、サーバーに完了通知（POST /api/upload/complete）
7. サーバーがメタデータを保存
```

## 制約事項
- ブラウザによってはHEIC形式の変換に時間がかかる場合がある
- 大量の画像（30枚以上）をアップロードする場合、圧縮処理に数秒かかる
- オフライン環境では動作しない
- 方式2の場合、LocalStackまたはAWS S3の設定が必要

## セキュリティ

### 共通
- ファイル拡張子の偽装を防ぐため、MIMEタイプを厳密にチェック
- セッションIDはUUIDv4を使用してパス推測を防止

### 方式1の場合
- アップロードされたファイルはサーバーサイドでも再バリデーション
- ファイルサイズ制限を厳格に設定

### 方式2の場合（推奨）
- 署名付きURLの有効期限を短く設定（5分）
- CORS設定で許可されたオリジンのみアップロード可能
- S3バケットポリシーで適切なアクセス制御
- アップロード完了前のファイルは定期的にクリーンアップ

## テスト

### E2Eテスト

**詳細は `docs/E2E_TESTING.md` を参照してください。**

#### テストケース

**正常系**:
1. 3枚の画像をアップロードできる
2. 50枚の画像をアップロードできる
3. 画像の並び替えができる（ドラッグ&ドロップ）
4. 個別削除ができる
5. 確認画面で正しく表示される
6. 圧縮処理のプログレスバーが表示される

**異常系**:
1. 2枚以下の場合エラーメッセージが表示される
2. 51枚以上の場合エラーメッセージが表示される
3. 非対応フォーマットの場合エラーメッセージが表示される
4. ファイルサイズ超過の場合エラーメッセージが表示される

#### テスト実行

```bash
# 高速モード（MSWモック）
npm run test:e2e:fast -- upload.spec.ts

# フルモード（LocalStack使用）
npm run test:e2e:full -- upload.spec.ts
```

#### テストファイル
- `tests/e2e/upload.spec.ts`: 画像アップロードE2Eテスト
- `tests/fixtures/test-image-*.jpg`: テスト用画像ファイル

### ユニットテスト

#### テスト対象
- `app/utils/imageCompression.ts`: 圧縮ロジック
- `app/utils/imageValidation.ts`: バリデーションロジック
- `app/utils/s3Upload.ts`: S3アップロード処理

#### テストケース
1. 画像圧縮が正しく動作する
2. HEIC→JPEG変換が正しく動作する
3. バリデーションが正しく動作する（枚数、形式、サイズ）
4. 署名付きURL取得が正しく動作する
5. S3アップロードが正しく動作する

---

**作成日**: 2026-01-07
**機能優先度**: High
**依存機能**: なし
